package io.github.bect.fieldweather.viewmodel

import android.app.Application
import android.content.Context
import androidx.lifecycle.AndroidViewModel
import androidx.lifecycle.viewModelScope
import io.github.bect.fieldweather.data.AppDatabase
import io.github.bect.fieldweather.data.SavedLocation
import io.github.bect.fieldweather.data.SettingsRepository
import io.github.bect.fieldweather.data.WeatherRecord
import io.github.bect.fieldweather.location.LocationHelper
import io.github.bect.fieldweather.network.SyncRequest
import io.github.bect.fieldweather.network.WeatherApi
import io.github.bect.fieldweather.network.WeatherRecordDto
...
class WeatherViewModel(application: Application) : AndroidViewModel(application) {
    ...
    init {
        // Load initial forecast from cache for faster UI
        viewModelScope.launch {
            settings.activeLocationName.collectLatest { locName ->
                if (locName != null) {
                    _forecastResult.value = settings.getForecastCache(locName)
                } else {
        ...
        // Background calculation for Markov Chain forecast
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.Default) {
            combine(allRecords, activeLocation) { records, location ->
                if (location == null) return@combine null
                val locRecords = records.filter { it.locationName == location.locationName }.sortedBy { it.timestamp }
                if (locRecords.isEmpty()) return@combine null

                if (locRecords.size == 1) {
                    return@combine Pair(location.locationName, ForecastResult(locRecords.last().condition, 100.0, "0th-Order Baseline"))
                }

                val lastRecord = locRecords.last()
                val prev1 = lastRecord.condition
                val prev2 = locRecords[locRecords.size - 2].condition

                var bestCondition = "Unknown"
                var maxCount = 0
                var totalTransitions = 0
                var model = 2

                // Try 2nd Order
                val transitions2nd = mutableMapOf<String, Int>()
                for (i in 2 until locRecords.size) {
                    if (locRecords[i - 2].condition == prev2 && locRecords[i - 1].condition == prev1) {
                        val next = locRecords[i].condition
                        transitions2nd[next] = transitions2nd.getOrDefault(next, 0) + 1
                    }
                }
                
                if (transitions2nd.isNotEmpty()) {
                    val maxEntry = transitions2nd.maxByOrNull { it.value }
                    if (maxEntry != null) {
                        bestCondition = maxEntry.key
                        maxCount = maxEntry.value
                        totalTransitions = transitions2nd.values.sum()
                    }
                } else {
                    // Fallback to 1st Order
                    model = 1
                    val transitions1st = mutableMapOf<String, Int>()
                    for (i in 1 until locRecords.size) {
                        if (locRecords[i - 1].condition == prev1) {
                            val next = locRecords[i].condition
                            transitions1st[next] = transitions1st.getOrDefault(next, 0) + 1
                        }
                    }
                    
                    if (transitions1st.isNotEmpty()) {
                        val maxEntry = transitions1st.maxByOrNull { it.value }
                        if (maxEntry != null) {
                            bestCondition = maxEntry.key
                            maxCount = maxEntry.value
                            totalTransitions = transitions1st.values.sum()
                        }
                    } else {
                        // Fallback to 0th Order
                        model = 0
                        val counts = locRecords.groupingBy { it.condition }.eachCount()
                        val maxEntry = counts.maxByOrNull { it.value }
                        if (maxEntry != null) {
                            bestCondition = maxEntry.key
                            maxCount = maxEntry.value
                            totalTransitions = locRecords.size
                        }
                    }
                }

                val probability = if (totalTransitions > 0) (maxCount.toDouble() / totalTransitions) * 100.0 else 0.0
                val modelStr = when (model) {
                    2 -> "2nd-Order Markov Chain"
                    1 -> "1st-Order Markov Chain"
                    else -> "0th-Order Baseline"
                }

                Pair(location.locationName, ForecastResult(bestCondition, probability, modelStr))
            }.collectLatest { pair ->
                if (pair != null) {
                    settings.saveForecastCache(pair.first, pair.second)
                    _forecastResult.value = pair.second
                }
            }
        }
        ...
        // Monitor latest record for 15-minute logic
        viewModelScope.launch {
            dao.getLatestRecord().collectLatest { latest ->
                if (latest != null) {
                    val latestTime = Instant.parse(latest.timestamp).toEpochMilli()
                    val currentTime = System.currentTimeMillis()
                    val waitMillis = 15 * 60 * 1000L
                    if (currentTime - latestTime < waitMillis) {
                        _timeUntilNextLog.value = waitMillis - (currentTime - latestTime)
                    } else {
    ...
    fun resetSyncStatus() {
        viewModelScope.launch {
            dao.resetSyncStatus()
            _lastSyncedDate.value = null
            getApplication<Application>().getSharedPreferences("FieldWeatherPrefs", Context.MODE_PRIVATE).edit().remove("last_synced_date").apply()
        }
    }

    fun syncData() {
        val provider = settings.syncProvider.value
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                if (provider == "Turso Cloud") {
                    val dbUrl = settings.tursoDbUrl.value
                    val token = settings.tursoToken.value
                    if (dbUrl.isBlank() || token.isBlank()) {
                        _syncState.value = SyncState.Error("Turso credentials missing")
                        return@launch
                    }
                    
                    val tursoRepo = io.github.bect.fieldweather.data.TursoRepository(dbUrl, token)
                    val result = tursoRepo.syncRecords(unsynced)
                    ...
                    if (result.isSuccess) {
                        ...
                        _syncState.value = SyncState.Success("Synced ${updatedRecords.size} records to Turso")
                    } else {
                        _syncState.value = SyncState.Error(result.exceptionOrNull()?.message ?: "Turso sync failed")
                    }
                } else {
                    val currentApi = api
                    if (currentApi == null) {
